...
class DataBowl3Detector(Dataset):
    def __init__(self, data_dir, split_path, config, phase='train', split_comber=None):
        ...
        idcs = split_path # np.load(split_path) #subset0/...
    ...
    def __getitem__(self, idx,split=None):
        ...
        if self.phase != 'test':
            if not isRandomImg:
                bbox = self.bboxes[idx]#num,x,y代,z,r,第一个参数表的是第几个CT序列（trainlist排列好的）后面代表的是xyz和直径。当直径大于2.5写1行，大于10写1+2行，大于20写1+2+4行
            ...
            imgs = np.load(self.filenames[idx]) #[1,331,236,323]
            # print('imgs.shape',imgs.shape)
            bboxes = self.sample_bboxes[idx]#时他们能被stride整除
            nz, nh, nw = imgs.shape[1:]#(197, 181, 224)
            pz = int(np.ceil(float(nz) // self.stride)) * self.stride
            ph = int(np.ceil(float(nh) // self.stride)) * self.stride
            pw = int(np.ceil(float(nw) // self.stride)) * self.stride
            imgs = np.pad(imgs, [[0,0],[0, abs(pz - nz)], [0, abs(ph - nh)], [0, abs(pw - nw)]], 'constant',constant_values = self.pad_value)#能被stride整除
            # print('1------pad',self.stride, pz,ph,pw)
            xx,yy,zz = np.meshgrid(np.linspace(-0.5,0.5,imgs.shape[1]//self.stride), #linspace 创建等差数列
                                   np.linspace(-0.5,0.5,imgs.shape[2]//self.stride),#当然我们可以指定多个参数，比如三个参数，那么我们的就可以用三个一维的坐标轴上的点在三维平面上画网格。
                                   np.linspace(-0.5,0.5,imgs.shape[3]//self.stride),indexing ='ij')
            coord = np.concatenate([xx[np.newaxis,...], yy[np.newaxis,...],zz[np.newaxis,:]],0).astype('float32')  #newaxis 新增一个维度
            # print('------1',imgs.shape)
