...
class DPN(nn.Module):
    ...
    def forward(self, x, coord):
        if debug: print('x.size()', x.size(), 64)#, coord.sizex = {Tensor}  Show Value
        out0 = F.relu(self.bn1(self.conv1(x)))
        if debug: print ('out0.size()', out0.size())
        out1 = self.layer1(out0)
        if debug: print ('out1.size()', out1.size())
        out2 = self.layer2(out1)
        if debug: print ('out2.size()', out2.size())
        out3 = self.layer3(out2)
        if debug: print ('out3.size()', out3.size())
        out4 = self.layer4(out3)
        if debug: print ('out4.size()', out4.size())

        out5 = self.path1(out4)
        if debug: print ('out5.size()', out5.size(), torch.cat((out3, out5), 1).size())
        outs=torch.cat((out3, out5), 1)
        out6 = self.layer5(torch.cat((out3, out5), 1))
        if debug: print ('out6.size()', out6.size())
        out7 = self.path2(out6)
        print ('8', out2.size(),out7.size(), coord.size(),torch.cat((out2, out7), 1).size())#torch.cat((out2, out7, coord), 1).size()
        out8 = self.layer6(torch.cat((out2, out7, coord), 1))
        if debug: print ('out8.size()', out8.size())
