...
class Net(nn.Module):
    ...
    def forward(self, x, coord):
        ...
        comb3 = self.back1(torch.cat((rev3, out3), 1))#torch.Size([7, 64, 12, 12, 12])#comb3 = self.drop(comb3)
        comb3 = self.back3(comb3)
        comb3 = self.back3(comb3)
        ...
        if coord.shape[0]==1:
            ...
            comb2 = self.back4(torch.cat((rev2, out2), 1))
        comb2 = self.back4(comb2)
        comb2 = self.back4(comb2)
...
class inRes(nn.Module):
    ...
    def __init__(self, n_in, n_out, stride=1):
        ...
        self.convr2 = nn.Conv3d(n_out, n_out, kernel_size=3, padding=1)
        self.bnr2 = nn.BatchNorm3d(n_out)

        self.convl1 = nn.Conv3d(n_in, n_out, kernel_size=1, stride=stride, padding=1)
        self.bnl1 = nn.BatchNorm3d(n_out)
        self.convl2 = nn.Conv3d(n_out, n_out, kernel_size=5, padding=1)
