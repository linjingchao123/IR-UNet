...
# find the mapping
# load groundtruth
antclscsv = pd.read_csv('D:/DeepLung-master - 没改/nodcls/data/annotationdetclsconvfnl_v3.csv', \
...
tedetpath = 'D:/DeepLung-master - 没改/nodcls/data/predanno0.1.csv'
# fid = open(tedetpath, 'r')#fold = 0
prdcsv = pd.read_csv(tedetpath)
...
pbbpth = 'D:/DeepLung-master - 没改/nodcls/data/bbox/'
...
import os
import argparse
from models.new3d import DPN92_3D
...
# Cal mean std
preprocesspath = 'D:\DeepLung-master - 没改\$LUNA16PROPOCESSPATH\subset/'
preprocessalltrainpath = 'D:\DeepLung-master - 没改\$LUNA16PROPOCESSPATH\subset/'
preprocessalltestpath = 'D:\DeepLung-master - 没改\$LUNA16PROPOCESSPATH\subset/'
pixvlu, npix = 0, 0
...
for srsid, label, x, y, z, d in zip(tefnmlst, telablst, tecdxlst, tecdylst, tecdzlst, tedimlst):
    ...
    data = np.load(os.path.join(preprocessalltestpath, '1.3.6.1.4.1.14519.5.2.1.6279.6001.139577698050713461261415990027'+'_clean.npy'))#'/home/zhaojie/zhaojie/Lung/data/luna16/LUNA16PROPOCESSPATH/subset9/'
    # print data.shape
...
for srsid, label, x, y, z, d in zip(trfnmlst, trlablst, trcdxlst, trcdylst, trcdzlst, trdimlst):
    ...
    if srsid in blklst: continue
    # crop raw pixel as feature
    data = np.load(os.path.join(preprocessalltrainpath, '1.3.6.1.4.1.14519.5.2.1.6279.6001.139577698050713461261415990027'+'_clean.npy'))
    # print data.shape
    bgx = int(min(data.shape[1],max(0,x-CROPSIZE/2)))
    bgy = int(min(data.shape[2],max(0,y-CROPSIZE/2)))
    bgz = int(min(data.shape[3],max(0,z-CROPSIZE/2)))
