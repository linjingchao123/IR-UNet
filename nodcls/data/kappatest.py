...
for fd in [1,2,3,5]:#xrange(1,4,1):
	dctprd = np.load('D:\DeepLung-master - 没改/dctptlabel1.npy', allow_pickle = True).item()
	for d in range(4):
		modprd = np.load('D:\DeepLung-master - 没改/modprd'+str(d+1)+'fd1.npy').item()
		for k, v in dctprd.items():
			if v[d] != -1:
				assert v[d] in [0, 1]
