...
pdframe =  pd.read_csv('annotationdetclsconvfnl_v3-0.csv', names=['seriesuid', 'coordX', 'coordY', 'coordZ', 'diameter_mm', 'malignant'])
...
preprocesspath = []
for root, dirs, files in os.walk(r"D:\DeepLung-master - 没改\$LUNA16PROPOCESSPATH",topdown=False):
  for file in files:
    if 'clean' in file:
      file_path = os.path.join(root, file)
      preprocesspath.append(file_path)
print('len(preprocesspath)',len(preprocesspath))
savepath = './save/'
import os
import os.path 
...
for idx in range(len(newlst)):
	...
	for i in range(len(preprocesspath)):
		if pid in preprocesspath[i]:
			print(i)
			data = np.load(preprocesspath[i])
...
print('stop')
# train use gbt
subset1path = '../../$DOWNLOADLUNA16PATH/LUNA16/subset0/'
testfnamelst = []
