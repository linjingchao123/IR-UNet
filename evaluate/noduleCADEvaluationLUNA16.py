...
def evaluateCAD(seriesUIDs, results_filename, outputDir, allNodules, CADSystemName, maxNumberOfCADMarks=-1,
    ...
    if vis:
        if int(totalNumberOfNodules) > 0:
            ...
            plt.title('FROC - inresnet2')#%s' % (CADSystemName))
            if bLogPlot:
    ...
    #         if curfp == 16:
    #             break
    return (fps, sens, thresholds, fps_bs_itp, sens_bs_mean, sens_bs_lb, sens_bs_up) #(fps, sens, thresholds, fps_bs_itp, sens_bs_mean, sens_bs_lb, sens_bs_up)
...
def noduleCADEvaluation(annotations_filename,annotations_excluded_filename,
                        seriesuids_filename,results_filename,outputDir, vis=True, delimiter=',', clr = 'b', cap=''):
    '''
...
if __name__ == '__main__':
    ...
    results_filename = './inresnet2!/bbox/inresnet2!!!.csv'
    outputDir = './inresnet2!/'
